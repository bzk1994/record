...
`vue` 提供了 `mixins` 这个 `API`，可以让我们将组件中的可复用功能抽取出来，放到 `mixin.js` 中，然后在组件中引入 `mixin.js`，让组件显得不再臃肿，也提高了代码的可复用性。
...
`defaultStrat` 返回一个三元表达式，如果 `childVal` 为 `undefined`，就返回 `parentVal`，否则返回 `childVal`，这里主要以 `childVal` 优先，这也是为什么有 `component` > `mixins` > `extends` 这样的优先级。

`mergeField` 函数最后会将调用 `strat` 的结果赋值给 `options[key]`，这样不断修改 `options`。
